  ...
  /**
    ...
    */
    ...
    String result(GameSearch searcher){
    	ArrayList succs = this.getSuccessors(searcher);
    ...
    */
    ...
    ArrayList getSuccessors(GameSearch searcher){
		...
		for (int i=0;i<b.length;i++){//indexing the piles
			...
			if ((p>2)&&(i==0||p>b[i-1])) { //only split if its 3 or more & greater than the last
				float pf = (float)p;
